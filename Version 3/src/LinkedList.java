public class LinkedList {
    ...
    public String remove(PetNode productList, String product) throws Exception{
        LinkedList list = productList.getList();

        // set current as list head
        LinkedListNode current = list.head;
        ...
        // checks if head is product
        if (current.getProduct().equals(product)) {
            current = current.getNext();
            list.head = null;
            list.head = current;
        } else {
            LinkedListNode previous = list.head;  // keep track of previous node
            current = current.getNext();   // go to next node

            // loop through list
            while (!current.getProduct().equals(product)) {
                // go to next node
                previous = current;
                current = current.getNext();   
            }
